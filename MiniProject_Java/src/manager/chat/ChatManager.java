	...
	public void client() {
		...
		stpe.scheduleAtFixedRate(new Runnable() {
			...
			public void run() {
				...
				//System.out.println("list 사이즈="+list.size()); - 정상
				//System.out.println("frames갯수="+frames.size()); - 정상
				for (int i = 0; i < list.size(); i++) {
					//System.out.println("list사이즈="+list.size());
//					System.out.println("문의요청:"+list.get(i).getCheck());
//					System.out.println("문의완료:"+list.get(i).getComplited());
		...
		while (true) {
			...
			try {
				...
				if(ar.length>=2) { //회원으로부터 메시지가 왔을때,
					...
					//System.out.println("frames갯수"+frames.size());
					if(ar[1].equals("exit")) {
						for(int i=0; i<frames.size(); i++) {
							if(frames.get(i).getId().equals(ar[0])) {
								line = ar[0]+"님이 퇴장하셨습니다.";
								//frames.get(i).setMsg(ar[0]+"님이 퇴장하셨습니다.");	
//								frames.get(i).dispose();
								//frames.get(i).remove(i);
//								System.out.println("종료list 갯수"+list.size());
//								System.out.println("종료frames 갯수"+ frames.size());
//								System.out.println("종료frames 아이디"+frames.get(i).getId());
							}
						}
						//break;
					}
...
//						System.out.println(frames.get(i).getId());
						if(frames.get(i).getId().equals(ar[0])) {
